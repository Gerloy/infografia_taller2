...
function Init(){
    ...
    img_actual = Math.floor(Math.random()*imgs.length);seleccionada = false;
}
...
function Click(a){
    //img_actual = Math.floor(Math.random()*imgs.length);
    modulo.getPantallaActual().metodos.invokeFunction("Selecciono");
    time = 0;
    seleccionada = true;
}
...
function getSeleccionada(){return seleccionada;}
